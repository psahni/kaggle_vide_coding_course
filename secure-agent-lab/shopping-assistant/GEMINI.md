...
- **ADK Workflow & Agent Structure**: When defining workflows and agents, structure `app/agent.py` using `LlmAgent` and `Workflow`. Define the graph edges using python tuple notation (e.g., `edges=[('START', shopping_agent)]`) instead of calling helper methods like `Edge.chain`. Ensure imports are structured as:
